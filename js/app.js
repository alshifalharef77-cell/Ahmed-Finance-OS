...
function walletName(code) { return walletByCode(code)?.name || wallets.find(wallet => wallet.code === code)?.name || 'Other'; }
...
async function loadData() {
  ...
  transactions = rows.flatMap((items, index) => items.map(item => ({ ...item, type: types[index], walletCode: item.walletCode || walletByName(item.wallet)?.code || preferences.lastWallet || 'o' })));
}
...
async function manageWallet(parts) { const action = (parts.shift() || 'list').toLowerCase(); if (action === 'list') { print(`WALLETS\n${divider()}\n${wallets.map(item => `${pad(item.code.toUpperCase(), 5)} ${pad(item.name, 20)} ${item.hidden ? 'HIDDEN' : 'ACTIVE'}`).join('\n')}`, 'muted'); return; } if (action === 'add') { const [code, ...names] = parts, name = names.join(' '); if (!/^[a-z0-9]$/i.test(code || '') || !name || wallets.some(item => item.code === code.toLowerCase())) { print('Usage: wallet add <one-letter-code> <name>', 'error'); return; } await add('wallets', { code: code.toLowerCase(), name }); await changed('wallet'); print(`Wallet added: ${name}`, 'success'); return; } const target = walletByCode(parts[0]); if (!target) { print('Error: wallet code not found.', 'error'); return; } if (action === 'rename') { const name = parts.slice(1).join(' '); if (!name) { print('Usage: wallet rename <code> <new name>', 'error'); return; } await update('wallets', target.id, { name }); } else if (action === 'hide') await update('wallets', target.id, { hidden: true }); else { print('Use: wallet [list|add|rename|hide]', 'error'); return; } await changed('wallet'); print('Wallet updated.', 'success'); }
...
async function undo() { if (!lastUndo) { print('Nothing to undo in this session.', 'warning'); return; } const item = lastUndo; if (item.kind === 'add') await softDelete(item.store, item.record.id); else if (item.kind === 'restore') await update(item.store, item.id, { deleted: false }); else if (item.kind === 'update') await update(item.store, item.id, item.before); lastUndo = null; await changed('undo'); print('Last change undone.', 'success'); showDashboard(); }
...
async function execute(raw, echo = true) { if (pendingDueMatch) { if (echo) print(`ahmed@finance:~$ ${raw}`, 'command'); await handleDueMatch(raw); return; } if (editDraft) { if (echo) print(`ahmed@finance:~$ ${raw}`, 'command'); await continueEdit(raw); return; } if (entryDraft) { if (echo) print(`ahmed@finance:~$ ${raw}`, 'command'); await continueEntry(raw); return; } if (dueDraft) { if (echo) print(`ahmed@finance:~$ ${raw}`, 'command'); await continueDue(raw); return; } if (dueEditDraft) { if (echo) print(`ahmed@finance:~$ ${raw}`, 'command'); await continueDueEdit(raw); return; }
  ...
  if (action === 'fuel' && parts[0] === 'stats') fuelStats();
